...
### <7> 2개 이상의 html 안에서 동일한 자바스크립트 코드를 자주 사용할 경우 자바스크립트 코드의 유지 보수가 힘들다. 즉 자스코드가 변경 되면 모든 html 을 열어서 자스 코드를 수정해야한다. 해결책은?
```
	자스코드를 xxx.js 파일에 저장하고 각 html 에서 아래처럼 수입하면된다.
	---------------------------------
	<script src = "xxx.js"></script>
	---------------------------------
		xxx.js 파일안의 모든 자스 코드를 script 태그 안에 삽입하라.
```

### <8> 자바스크립트는 프로그래밍 언어이다. 자스에서 자료형(=데이터의 종류=data type)의 종류가 뭘까?
```
	---------------------------------
	(1) 숫자형
	---------------------------------
	...
	(4) 함수형
	---------------------------------
		▶function(){실행구문;} 의 형태의 데이터를 말한다.
		▶function(){실행구문;} 의 형태를 익명함수라고 부른다.
		▶<예> var doubleNum = function(num){ return num*num; };
	---------------------------------
	(5) 객체형
	---------------------------------
		▶객체가 차지하는 메모리 공간의 위치 주소값(=메위주)을 말한다.
...
### <9> 자바스크립트에서 변수 선언은 어떻게 합니까?
```
	----------------------------------------------------------
	변수 정의 => 데이터가 저장되는 [메모리의 기억장소]에 붙인 이름을 말한다.
				<비유> 음식을 담는 그릇과 동일한 개념이다.
	----------------------------------------------------------
	변수 사용 이유 => 데이터를 변수에 저장함으로써 [재사용]이 가능하다.
...
-----------------
	...
	▶형태
		...
		-----------------------------
		대괄호[]는 생략이 가능하다는 의미이다.
		매개변수는 0개 이상 나올수 있음. 2개 이상이라면 ,로 나열함.
		매개변수 왼쪽에 var가 있으면 절대 안된다.
		-----------------------------
		...
		●<주의>동일한 이름의 정적함수가 2개 이상 선언되면 마지막 정적함수가 진짜 정적함수다.
		-----------------------------
-----------------
익명 함수
-----------------
...
--------------
	▶확인 버튼과 취소 버튼이 있는 [경고상자]를 보여준다.
		[확인상자]를 보여주고 [확인] 버튼을 누르면 ture 리턴, [취소] 버튼을 누르면 false 리턴

--------------
prompt("문자열")
--------------
	▶[문자열입력상자]와 확인 버튼, 취소 버튼이 상자를 보여주고 [확인] 버튼 누르면 입력문자열 리턴	
...
--------------
	...
	▶익명함수 function(){자스코당;}은 n초 마다 필요해서 호출하고 있으므로 콜백함수이다.
...
--------------
	▶n초 후에 자스코딩을 실행한다. 시한 폭탄과 유사. <주의> "자스코딩"에서 "빼면 안된다.
	▶익명함수 function(){자스코당;}은 n초 마다 필요해서 호출하고 있으므로 콜백함수이다.
...
### <15> 자바스크립트에서 사용 가능한 객체 종류에는 뭐가 있을까?
```
------------------------
DOM(=Document Object Model) 객체
------------------------
	▶W3C 라는 단체가 [여러 브라우저 제작사]들과 협력하여 제작한 객체이다.
	▶대부분의 부라우저에서 공용으로 사용가능한 객체이다.
	▶DOM 객체의 기능
	...
	--------------------------------------------------------------------
		------------------------------------------------------------------
		xxx.html 파일안에 <input type="text" name ="uid"> 라는 태그가 있다면
		웹브라우저가 이 태그를 읽어들이면서 다음과 같은 진행을 가진다.
		------------------------------------------------------------------
			(1)웹브라우저가 <input type="text" name ="uid"> 를 읽어 들인다.
			(2)Text 객체가 생생된다. Text 객체의 속성변수에는 name,type,value 등이 있다.
...
------------------------
	...
	▶<예>Location 객체의 replace 라는 메소드를 호출하여 페이지 이동하기
		...
		---------------------------
			location 변수안에 Location 객체의 메모리 주소 위치값이 들어 있다.
...
자바스크립트 내장 객체
------------------------
	▶자바스크립트 차원에서 미리 만들어 저장된 [생성자 함수]로부터 만들어지는 객체 말한다.
	▶각종 데이터를 생산/관리한다.
	------------------------------------------------------------------------
	<예> 지금 이 순간의 년도를 얻어서 thisYear 라는 변수에 저장하고 싶다면
		아래처럼 Date 객체를 생성하고 getFullYear 메소드를 호출하면 된다.
	------------------------------------------------------------------------
		var today = new Date();
		var thisYear = today.getFullYear();
	------------------------------------------------------------------------
	▶<예> 다량 데이터를 저장하고 관리하고 싶다면 아래 처럼 Array 객체를 생성하면 된다.
	------------------------------------------------------------------------
...
------------------------
	...
	▶장점
		...
		▶개발자, 디자이너, 퍼블려셔를 모두 만족하는 강력한 기능을 가지고 있다.
			즉 짧은 코딩으로 원하는 일을 쉽게 처리한다.
	▶단점
		▶정해진 코딩 패턴이 없다. 즉 개발자 마다 다양한 코딩을 할 수 있어서  유지보수가 힘들다.
			그래서 요즘 react 를 사용하고 있다.
		▶<참고>react
			자바스크립트로 화면 UI 를 만들 수 있는 독단적인 플랫폼이다.
			nodeJS 를 기반으로 작동된다.
...
------------
	...
	▶ 객체 생성하고 데이터 저장하는 방법
		...
		<방법1>
		------------
			var 변수명 = new Array(데이터1, ~, 데이터n) //잘 안쓰는 방법
		------------
		<방법2>
		------------
			var 변수명 = new Array();
	...
	▶Array 객체의 메소드 종류
		...
		--------------
		pop()		=> 마지막 배열변수안의 데이터를 리턴하고 마지막 배열변수를 제거함. 오려내기와 비슷
		--------------
		concat( 다른 Array객체)	=> 현 Array 객체와 다른 Array 객체의 배열변수를 복사해서 새로운 Array 객체 안에
							저장하고 새로운 Array 객체를 리턴함. 2021.07.07_01_Array 객체 생상도 그림 참조.
		--------------
		sort()		=> 저장된 데이터를 [문자열] 취급해 [오름차순] 정렬.
					숫자가 아닌 문자가 저장되어 있을때 정렬 시 사용하는 메소드이다.
					----------------------------------------------------------------------
					var kors = ["김소희","성이현"","변희원"]; kor.sort(); //정렬순서는 김소희" "변희원" "성이현" 이다.
					----------------------------------------------------------------------
		...
		------------------------------------------------------------------------------------------
			▶저장된 숫자 데이터를 오름차순 정렬	
			▶만약 문자열을 정렬하고 싶다면 위에 있는 sort() 써라.
		...
		--------------------
			▶[시작인덱스번호] 부터 [끝인덱스번호 이전]까지 [복사해서] [새로운 Array 객체]로 리턴


		--------------------
		splice(시작인덱스번호, 개수)
		--------------------	
			▶지정한 [시작 인덱스번호] 부터 [개수]만큼 오려내서 [새로운 Array 객체]로 리턴.

	▶Array 객체의 속성변수
		---------------------------------------------
		length		=> 배열변수의 개수가 저장되어 있음.
		---------------------------------------------
****************************
Date 객체
****************************
	...
	---------------------------------------------
		...
		<주의>Date 객체는 월을 0~11 까지의 숫자로 관리하므로 항상 세팅 시 월을 -1 한 다음 삽입한다.
	-------------------------------------------------------------
	●Date 객체의 메소드 (리턴값 정수!!!)
	-------------------------------------------------------------
		...
		getTime()		=> [1970년1월1일0시0분0초] ~ [Date 객체의 기억날짜] 까지의 차이를 초*1000 으로 리턴.
						...
						var dinterval = date2.getTime()-date1.getTime()/1000;
	...
	-------------------------------------------------------------
		...
		----------------------------------------------------------------
		현재 지금 날짜에서 올해 크리스마스 날까지 며칠 남았는지 구해서 변수 interval 에 저장하기
		----------------------------------------------------------------
		var today = new Date();
		...
		----------------------------------------------------------------
		var date = new Date(2031,2,1-1)
		var xxx = date.getDate();
		----------------------------------------------------------------
		오늘부터 1000일 후의 년월일을 구해 변수 year,month,date 에 저장하기
		----------------------------------------------------------------
		var xxx_day = new Date();
		xxx.setDate(xxx_day.getDate()+1000);
		var year = xxx.getFullYear()
		var month = xxx.getMonth()+1
		var date = xxx.getDate()
		데이터객체는 0부터 시작하기 때문에 1을 더해야 우리가 달이 나온다.
		----------------------------------------------------------------
		현재 지금 날짜에서 요일을 얻어서 변수 week 에 저장하기. 단 요일은 한글로 저장하기
		----------------------------------------------------------------
		var today = new Date();
...
****************************
	...
	------------------------------------------------
		...
		▶charAt(인덱스번호)
		---------------------------------------------
			=>인덱스번호에 위치하는 문자 한개 복사해서 리턴. 인덱스번호는 0부터 시작한다.
			=>var uname="성이현"; var firstName = uname.charAt(0);
		---------------------------------------------
		▶concat("새로운문자열")
		---------------------------------------------
			=>String 객체가 소유한 문자열을 복사해 [새로운 문자열]을 붙혀 새로운 String 객체로 리턴.
		---------------------------------------------
		▶indexOf("찾는문자",검색시작인덱스번호)		*****
		---------------------------------------------
			=>String 객체가 소유한 문자열에서 찾는문자가 위치한 곳에 인덱스 번호를 리턴.
				단 검색 시작점은 검색시작인덕스번호 부터 검색한다.
				단 검색 방향은 왼쪽에서 오른쪽으로 검색한다. 즉 앞에서부터 검색한다.
			...
			-----------------------------------------------------
			var fruit ="수박 참외 파인애플 복숭아"; 에서 fruit에 복숭아가 있으면 xxx 변수에 "있음" 저장하기
			-----------------------------------------------------
		...
		---------------------------------------------
			=> String 객체가 소유한 문자열에서 찾는문자가 위치한 곳에 인덱스 번호를 리턴.
				단 검색 시작점은 검색시작인덕스번호 부터 검색한다.
				단 검색 방향은 오른쪽에서 왼쪽으로 검색한다. 즉 뒤에서부터 검색한다.	
		...
		----------------------------------------	
			=> 시작인덱스번호 부터 끝인덱스번호 이전 까지 문자열을 복사해 리턴.
			끝인덱스번호를 생략하면 마지막 인덱스번호까지 리턴
		...
		<문> 임의의 아이디 "abc123" 가 있다. 
		     첫 글자가 영문소문자이어야 하고, 숫자와 영문소문자로 구성되야하고
		...
		var xxx = "적합";
		----------------------------------------------------
		//아이디 첫글자가 영어가 아니면 cnt 변수 1업그레이드하기
		----------------------------------------------------
...
****************************
	...
	-----------------------------------
		...
		max( 숫자1, ~, 숫자n )	=> 숫자 1부터 숫자n 까지 중 최대값을 리턴.
		min( 숫자1, ~, 숫자n )	=> 숫자 1부터 숫자n 까지 중 최소값을 리턴.
		random( )				=> 0 이상 1 미만의 무작위 실수를 리턴.
		abs(x)					=> x 값의 절대값을 리턴.
		pow( x, y )				=> x 의 y승을 리턴. <예> pow(3,2) 리턴값 9
...
RegExp 객체
****************************
	●문자열의 패턴을 관리하는 객체이다. <참고>RegExp는 Regular Expression(정규 표현식)의 약어 이다.
	●객체 생성 방법
		-------------------------------------
	...
	●RegExp 객체의 메소드
		-------------------------------------
		test("문자열")	=> 문자일이 패턴식을 만족하면 true 리턴, 아니면 false 리턴
		-------------------------------------
	●예
		-----------------------------------------
		// 변수 uid 에 아이디 "abc123"를 저장
		var uid = "abc123"
		----------------------------------------
		//RegExp 객체 생성해서 변수 regExp 에 저장하기
		//RegExp 객체가 관리하는 패턴식은 아래와 같다.
		//첫글자는(^사용) 영어소문자, 두번째 부터는 영소문 또는 숫자 또는 _로 하고
		// 4~9 사이로 반복하기($). 그리고 이런 패턴으로 끝나기($)
	...
	●패턴식 예
		...
		공백 존재			=> /[ ]+/
		----------------------------------------------------------------------
		email 형식			=> /^(([0-9a-zA-Z_-]+)@([0-9a-zA-Z_-]+)(\.[0-9a-zA-Z_-]+){1,2}$)/
		----------------------------------------------------------------------
		임의 문자 3개로 만 구성	=> /^...$/
		----------------------------------------------------------------------
		abc로 시작하고 xyz 로 끝남	=>/^(abc).*(xyz)$/
		----------------------------------------------------------------------
	...
	●패턴식 키워드
		...
		[문자열]	=> 문자열 중에 문자 하나.
		--------------------------------------------------
		[^문자열]	=> 문자열이 모두 아닌(not) 문자 하나.. [^abc] a,b,c가 아닌 문자 하나.
		--------------------------------------------------
		...
		--------------------------------------------------
		\w			=> [a-ZA-Z0-9_]와 동일. 영소문자 또는 영대문자 숫자 또는 언더바 중에 하나.
		--------------------------------------------------
		\d			=> [0-9]와 동일. 숫자 하나.
		--------------------------------------------------
		\s			=> [ ]과 동일. 숫자 하나.
		...
		[.]			=>  점 문자 의미.
		--------------------------------------------------
		{n}			=> 왼쪽에 데이터가 n개만 나옴. [a-z]{3} => 영소문자 3개.
		--------------------------------------------------
		{n,}		=> 왼쪽 데이터가 n개 이상 나옴. {,m}은 없음
		--------------------------------------------------
		{n,m}		=> 왼쪽 데이터가 n ~ m개 나옴.
		--------------------------------------------------
		*			=> 왼쪽 데이터가 0개 이상 나옴.	[a-z]* => 영소문자 0개 이상 나옴.
		--------------------------------------------------
		+			=> 왼쪽 데이터가 1개 이상 나옴.	[a-z]+ => 영소문자 1개 이상 나옴.
		--------------------------------------------------
		?			=> 왼쪽 데이터가 0~1개 나옴.	[a-z]? => 영소문자 0~1개 나옴.
		--------------------------------------------------
		|			=> | 좌우 문자열 중 하나 리턴.
		--------------------------------------------------
		()			=> 괄호 안을 하나로 묶어 관리함. 즉 그룹화.
		--------------------------------------------------
```
...
```
	---------------------------------------------------------
	자스에서 [예외]란 웹브라우저가 [감지]할 수 있는 오류를 말한다.
	---------------------------------------------------------
	예외가 발생하면 예외를 관리하는 객체가 생선된다.
	예외를 관리하는 객체의 메소드나 속성변수를 호출하여 예외의 정보를 알 수 있다.
	---------------------------------------------------------
	<주의>감지할 수 없는 오류는 예외가 아니다. <예> 전원꺼짐
	---------------------------------------------------------
```
### <18> 예외가 발생하면 대부분 프로그램이 중단되는데 중단 시키지 않고 프로그램을 진행할 수 없나요?
```
	예외 발생할 경우 try{}catch(e){~}finally{} 구문을 
	사용하여 프로그램의 갑작스런 중단을 막고 
	원하는 방향으로 프로그램을 진행하는 것을 말한다.
	이런 처리를 예외 처리(=Exception Handling)라고 한다.
	---------------------------------
...
### <20> BOM(=Brwser Object Model) 객체의 종류와 메소드/속성변수 정의
```
	●브라우저 제작사 별로 자체 제작된 객체를 말한다.
	------------------------------
	●객체 생성, 속성/메소드 호출 형식
	------------------------------
		객체 생성 코드 		=> 없음. 이미 객체는 생성되어 있고 심지어 변수까지 만들어 객체를 저장하고 있다.
	...
	-------------------------
		...
		----------------------------------
			...
			blur( )
				▶웹화면에서 초점을 제거
			moveTo(x좌표, y좌표)
				▶웹화면을 지정한 [절대적 위치]로 이동. 
					x좌표는 웹화면 왼쪽에서 떨어진 거리
					y좌표는 웹화면 위쪽에서 떨어진 거리
			resizeTo(너비, 높이)
				▶웹화면을 지정한 절대적 크기로 변경
		------------------------------------
		Window 객체 속성변수
		------------------------------------
			...
			onload
				▶body 태그 안의 내용물을 다 읽어들인 후 실행할 익명함수을 저장한다.
				---------------------------------------------------------------
				<예> window.onload = function(){alert("문자열");}
				---------------------------------------------------------------
	...
	----------------------
		...
		---------------------------------
			...
			**href      => 웹페이지의 URL 주소를 알려주거나 지정한  URL 주소의 페이지를 열어준다.
			...
			hostname	=> URL 주소의 호스트 이름과 포트를 알려준다.
	...
	*********************
		------------------------------------------------------------
		웹브라우저의 History List(방문한 적이 있는 URL 주소)를 관리한다.
		------------------------------------------------------------
	...
	*********************
		...
		---------------------------
			button		=> 클릭한 마우스 버튼의 고유 번호가 저장됨.
							(1번-왼쪽버튼, 2번-오른쪽버튼, 3번-가운데버튼. 마우스 제조사별로 버튼 번호가 약간 다르다.)
			keyCode**	=> 누른 키보드의 고유 번호가 저장됨 예> 엔터키 치면 keyCode 에 13이 저장된다.
			clientX**	=> 웹브라우저 화면의 X좌표 위치가 저장된다. 스크롤은 무시한다. 죄측 상단 맨 위를 0,0으로 본다.
			clientY**	=> 웹브라우저 화면의 Y좌표 위치가 저장된다. 스크롤은 무시한다. 죄측 상단 맨 위를 0,0으로 본다.
			screenX		=> 각자 모니터 화면 전체를 기준으로 이벤트가 발생한 X좌표가 자장됨
			screenY		=> 각자 모니터 화면 전체를 기준으로 이벤트가 발생한 Y좌표가 자장됨
			altKey		=> alt 키의 누름 상태 여부가 true, false 값으로 저장됨
			ctrlKey		=> ctrl 키의 누름 상태 여부가 true, false 값으로 저장됨
			shiftKey	=> shif 키의 누름 상태 여부가 true, false 값으로 저장됨
			type		=> 이벤트의 종류가 저장됨
			기타등등
	...
	*********************
		-------------------------------------------------------------
		현재 화면의 해상도, 색상, 화면의 크기 등에 관한 정보를 제공한다.
		-------------------------------------------------------------
	...
	***********************************
		...
		-----------------------
			...
			location		=> 현재 페이지의 URL 주소를 문자열 행태로 저장하고 있다.
...
```
	---------------------------------------------------
	●W3C 라는 단체가 여러 웹브라우저 제작사 들과 협력하여 제작된 객체이다.
	---------------------------------------------------
	●DOM 객체는 주로 html 에서 [태그]를 관리하는 객체이다.
	---------------------------------------------------
	●[웹브라우저]가 html 파일 안의 태그들을 읽어 [객체]를 생성하고
	이 [객체]에 대응하는 [컨텐츠]를 [웹브라우저 화면]으로 출력한다.
	---------------------------------------------------
	...
	*****************************
		...
		Form 객체 속성변수
		---------------------------
			action***	=> form 태그 안의 입력양식에 입력(선택)된 데이터를 보낼 [웹 서버 쪽 URL 주소](DB연동 간접 명령어)를 저장된다.
			method***	=> 웹 서버로 데이터를 보내는 방법을 저장한다.
							get		=> 입력(선택) 데이터가 URL 뒤에 붙어 전송(보안성)
		...
		---------------------------
			...
			reset()***	=> form 태그 안의 입력 양식에 입력(선택)된 모든 내용을 초기값으로 변경한다
							즉 데이터를 비우거나 체크를 풀어버린다.
	*****************************
	●Text 객체
	*****************************
		---------------------------------------------------------
		<input type="text" name="xxx" ~> 태그를 관리하는 객체이다.
		위 태그가 실행되어 웹브라우저에 출력될 때 [문자입력상자]라고 부른다.
		---------------------------------------------------------
		...
		-----------------
			...
			select()	=>[문자입력상자] 안을 드레그 하여 입력 내용을 선택하도록  한다.
	...
	*****************************		
		...
		위 태그가 실행되어 웹브라우저에 출력되지는 않는다. [숨겨진문자입력상자]라고 부른다.
	...
	*****************************	
		...
		위 태그가 실행되어 웹브라우저에 출력되지는 않는다. [파일입력상자]라고 부른다.
		---------------------------------------------------------
		---------------------------------------------------------
		속성변수 호출 	=>  document.form태그name값.xxx.속성변수
		---------------------------------------------------------
	...
	*****************************
		...
		위 태그가 실행되어 웹브라우저에 출력되지는 않는다. [라디오버튼]라고 부른다.
		---------------------------------------------------------
		---------------------------------------------------------
		name값이 같은 라디오 버튼의 개수가 2개 이상일 경우 속성변수 호출 	=>  document.form태그name값.xxx[i].속성변수
		---------------------------------------------------------
		name값이 같은 라디오 버튼의 개수가 2개 이상일 경우 메소드 호출 		=>  document.form태그name값.xxx[i].메소드(~)
		---------------------------------------------------------		
		---------------------------------------------------------
		name값이 같은 라디오 버튼의 개수가 1개 이상일 경우 속성변수 호출 	=>  document.form태그name값.xxx.속성변수
		---------------------------------------------------------
		name값이 같은 라디오 버튼의 개수가 1개 이상일 경우 메소드 호출 		=>  document.form태그name값.xxx.메소드(~)
		---------------------------------------------------------	
		------------------
		Radio 객체 속성변수
		------------------		
			value		=> 체크된 라디오 버튼 태그의 value 속성값이 저장되어 있다.
			checked		=> 체크 여부 정보가 자장되어 있다. 체크가 되어 있으면 true 가 저장되고 체크가 안되어 있으면 false 가 저장된다.
							<주의> 체크가 된 놈의 value 값이 서버로 전송된다.
									체크가 안된 놈의 value 값은 서버로 전송되지 않는다.
		------------------
		Radio 객체 메소드
		------------------
			click( ) => radio 단추의 체크를 넣어주거나 혹은 빼준다.
			---------------------------------------------------------
	...
	*****************************
		...
		위 태그가 실행되어 웹브라우저에 출력되지는 않는다. [체크박스버튼]라고 부른다.
		---------------------------------------------------------
		---------------------------------------------------------
		name값이 같은 체크박스의 개수가 2개 이상일 경우 속성변수 호출 	=>  document.form태그name값.xxx[i].속성변수
		---------------------------------------------------------
		name값이 같은 체크박스의 개수가 2개 이상일 경우 메소드 호출 		=>  document.form태그name값.xxx[i].메소드(~)
		---------------------------------------------------------		
		---------------------------------------------------------
		name값이 같은 체크박스의 개수가 1개 이상일 경우 속성변수 호출 	=>  document.form태그name값.xxx.속성변수
		---------------------------------------------------------
		name값이 같은 체크박스의 개수가 1개 이상일 경우 메소드 호출 		=>  document.form태그name값.xxx.메소드(~)
		---------------------------------------------------------	
		------------------
		Checkbox 객체 속성변수
		------------------		
			value		=> 체크된 체크박스 태그의 value 속성값이 저장되어 있다.
			checked		=> 체크 여부 정보가 자장되어 있다. 체크가 되어 있으면 true 가 저장되고 체크가 안되어 있으면 false 가 저장된다.
							<주의> 체크가 된 놈의 value 값이 서버로 전송된다.
									체크가 안된 놈의 value 값은 서버로 전송되지 않는다.
		------------------
		...
		------------------
			...
			---------------------------------------------------------
					document.xForm.skill[1].click()  코드가 실행되면 
						<input type="checkbox" name="skill" ~> 태그에 의해 출력되는 2번째 체크박스 버튼에 체크가 없으면 넣어주고
																체크가 있으면 체크 풀기
	*****************************
	●Select 객체
	*****************************
		...
		------------------	
			...
			multiple	=> 다중 선택 가능 여부가 저장되어 있다. true가 있으면 ctrl을 누르고 다중선택이 가능하다.
			oprions		=> Option 객체들을 저장한 Array 객체의 메모리 위치 주소값이 저장되어 있다.
		------------------
		Select 객체 메소드
		------------------
			focus()		=> 포커스를 들여 놓는다.
	...
	*****************************
		...
		---------------------------------------------------------
		속성변수 호출 	=>  document.form태그name값.select태그name값.option[인덱스번호].속성변수
		---------------------------------------------------------
		------------------
		Option 객체 속성변수
		------------------
...
```
	...
	-------------------
		-------------------------------------------------------------------------
		●특정 단체에서 만들어 무료도 제작되어 배포되는 [사용자 정의 객체] 중의 하나이다.
		-------------------------------------------------------------------------
		객체 이름은 대문자 J, 함수이름은 소문자 j
	**************************
	JQuery 장점
	**************************
		대부분의 브라우저에서 동일한 실행 결과를 가진다.(=크로스 브라우징=cross browsing)
	...
	**************************		
			...
			-----------------------------------------------------------------------------------
			쉬운 방법으로 에니메이션 관련 작업을 지원하는 JQuery 객체의 메소드가 존재한다.
			-----------------------------------------------------------------------------------
			쉬운 방법으로 [이벤트] 설정 및 [이벤트 처리]를 지원하는 JQuery 객체의 메소드가 존재한다.
			-----------------------------------------------------------------------------------
	**************************
	JQuery 객체의 메모리 위치 주소값를 얻는 방법
	**************************
		var uid = $("form [name=uid]").val();
		-----------------------------------------------------------------------------------
		jQuery("선택자") 또는 $("선택자") 라는 함수를 호출하면 리턴되는 데이터가 JQuery 객체의 메모리 위치 주소값이다.
		-----------------------------------------------------------------------------------
			
			-----------------------------------------------------------------------------------
			<예> var obj = $("[name=uid]"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 name="uid"를 가진 태그를 관리한다
														"[name=uid]" 가 바로 선택자이다.
			-----------------------------------------------------------------------------------
			<예> var obj = $(".pwd"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 class=pwd를 가진 태그를 관리한다
														".pwd" 가 바로 선택자이다.
			-----------------------------------------------------------------------------------
			<예> var obj = $("#addr"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 id=addr를 가진 태그를 관리한다
														"#addr" 가 바로 선택자이다.
			-----------------------------------------------------------------------------------
			<예> var obj = $(".family .relation"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 class=family를 가진 태그의 후손 즉 내부에
														class=relation 가진 태그 관리한다.
														".family .relation" 가 바로 선택자이다.
			-----------------------------------------------------------------------------------		
			<예> var obj = $("div"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 div 태그를 관리한다.
														"div" 가 바로 선택자이다.				
			-----------------------------------------------------------------------------------
			<예> var obj = $("xxx.div"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 class=xxx 가진 태그의 후손 중 div 태그 관리한다.
														"xxx.div" 가 바로 선택자이다.				
			-----------------------------------------------------------------------------------
			<예> var obj = $("div:eq(0)"); // 변수 obj 선언하고 $ 함수를 호출하여 JQuery 객체의 메위주를 얻어 변수 obj 저장
														이때 JQuery 객체는 1번째 div 태그를 관리한다.
														"div:eq(0)" 가 바로 선택자이다.				
			-----------------------------------------------------------------------------------
	...
	**************************
		...
		<2>HTML의 head 태그에 아래와 같이 수입하는 코드를 작성한다.
			...
			-------------------------------------------------------------
			jquery=x.x.x.min.js 안에는 $ 함수와 JQuery 객체의 메소드, 속성변수가 정의 되어 있다.
		-------------------------------------------------------------
...
```
함수는 단순한 단위 프로그램
객체는 조직화(속성변수와 메소드로) 된 단위 프로그램
	**************************************
	자바스크립트 사용자 정의 객체?
	**************************************
		●개발자가 직접 만든 [생성자 함수]로 부터 만들어지는 객체를 말한다.
		●결국 [생성자 함수]란 일반함수와 다른 객체 생성 목적으로 만들어진 함수이다.
			<참고>저자마다 [생성자함수]를 클래스라고 부르는 저자도 있다.
	...
	**************************************
		...
		------------------------------------------
			...
			메소드는 0개 이상 선언이 가능하다.
			실무적으로 속성변수 또는 메소드가 1개 이상 선언이 돼야한다.
			매개변수는 0개 이상 선언이 가능하고 2개 이상이면 매개변수명, 매개변수명, ~ 로 선언한다.
		------------------------------------------
		<예>
		------------------------------------------
		함수 이름의 첫문자가 대문자이면 생성자 함수라고 봐야한다.
	...
	**************************************
		...
		------------------------------------------
			<1> 변수를 선언한다.
			<2> new 에 의해 지정한 생성자함수를 메모리 공간으로 올려 객체화한다.
			<3> 메모리에 올려진 생성자 함수를 생성자함수( [데이터1] ) 형식으로 호출한다.
				이때 매개변수가 있다면 데이터1 이 매개변수로 전달된다.
...
*******************************
	...
	속성변수는 0개 이상 선언가능
	메소드는 0개 이상 선언가능
	실무적으로 속성변수나 메소드는 1개 이상 선언 해야한다.
	실무적으로 [일회성 사용자 정의 객체] 안에 메소드는 잘 나오지 않는다.
	주로 여러 데이터를 한곳에 모아 보관하고 사용할 목적으로 사용한다.
	-------------------------------------------------
	<예>
	-------------------------------------------------
	var sungjuk = {
	...
	***************************
	...
	-----------------------------
		...
		---------------------------
		변수명을 일일이 코딩
		다른 프로그램쪽을 데이터 전달하려면 모두 전해 주면 된다.
		그럼 받는쪽도 n개의 변수로 받아야한다.(=비효율적)
	-----------------------------
	<방법2> Array 객체에 저장한다.
	-----------------------------
		...
		alert(jumsu[0])
		---------------------------
		Array 객체에서 데이터를 꺼내는 방법
		---------------------------
		반복문을 사용하여 jumsu[i]를 코딩하면된다.
		다른 프로그램 쪽으로 데이터 전달하려면 Array 객체의 메위주만 전달해 주면된다.
		치명적 단점은 데이터가 무슨 데이터인지 알기가 힘들다.

		Array 객체에 데이터를 저장할 땐 단순한 일방적인 데이터를 저장한다.
		예>1000개의 사과의 무게를 저장할 때
			var appleWeight =[200,210,190,~]
	-----------------------------
	<방법3> 일회성 사용자 정의 객체에 저장한다.
	-----------------------------
		...
		장정은 데이터가 무슨 데이터인지 알 수가 있어 꺼내기 쉽다.
		단점은 반복문 사용이 힘들다.
	----------------------------------
	<방법2>는 데이터마다 구분이 필요없는(사과의 무게) 단순한 데이터 일 경우 사용
	<방법3>는 데이터마다 구분이 필요한(과목별 점수 등) 데이터 일 경우 사용
	---------------------------------------------------
	<문제>dept 테이블에 다음과 데이터가 저장되어 있을 경우 
			자스로 영역으로 데이터를 끌고 와서 관리한다면?
	---------------------------------------------------
		-----------------------
		dep_no 	dep_name   loc
		-----------------------
		...
		2차원 Array 객체를 사용하면
		위에 방법2와 같은 단점은 가지게된다.
		예를 들면 10, 20 등이 무슨 데이터인지 모른다는 것이다.


********************************
일회성 사용자 정의 객체의 메소드/속성변수
********************************
	변수.속성변수
	변수.메소드명(~)
```

### <25> [JSON]란?	******솔루션 화사 면접질문
```
●동종 또는 이기종 S/W 끼리 쉽게 데이터를 전달하기 위해 만든 [개방형 표준 데이터 표기법]을 말하다.
...
-----------------------------
	각종 프로그래밍 언어나 플랫폼(=platform)에 종속되지 않는 독립형 데이터 포맷이다.
		--------------------------
		<참고>플랫폼(=platform) = 특정 서비스를 하기 위한 기반. 보통 플랫폼은 무료이고 플랫폼이란 기반 위의 서비스는 유료
		--------------------------
...
********************************
	...
	스프링 같은 프레임워크 사용시 웹브라우저에서 [비동기 방식]으로 서버와 데이터를 주고받을 때 JSON을 사용하기도 한다.
		--------------------------
		<참고>비동기 방식
		--------------------------
			▷클라이언트의 웹브라우저에서 화면이동(=페이지 이동) 없이 AJAX기술로 웹서버와 통신하는 방식을 말한다.
			▷<예> 아이디 입력 후 버튼을 눌러 페이지 전환 없이 서버와 통신하여 아이디 중복여부를 수신 받는 경우
			▷<예> 네이버에서 검색 키워드 입력 시 페이지 전환 없이 서버와 통신하여 관련 검색어가 밑에 뜨는 경우
			▷<예> 네이버 지도에서 지도를 밀면 세로운 지도가 나오는 경우
		--------------------------
		<참고>동기 방식
		--------------------------
...
********************************
	...
	<주의> 키값은 서로 달라야한다.
	<주의> 데이터를 주로 문자열 또는 숫자 이다. 즉 모든 프로그램밍 언어가 공유하는 데이터이다. 
			이기종끼리 데이터를 공유하는데 프로그래밍언어의 고유 문법이 들어가면 안되기때문.
	...
	-----------------------------			
		변수명[키값]
		-------------------------
		alert(windowAirCondition["제폼이름"])
```
### <26> body onLoad = "openMesage()" 이 코드의 의미는?
```
	-----------------------------------------------------
	웹브라우저가 body 태그 안의 태그들을 모두 읽어 들인 후에
	자바스크립트의 openMesage 라는 함수를 호출해라.
	-----------------------------------------------------
	-----------------------------------------------------
	<참고> <태그명 on이벤트="자스코드">
	-----------------------------------------------------
		이 태그에 on 다음의 나오는 이벤트를 일으키면 자스코드를 실행하라.
```
### <27> 자스에서 var arr = []; 이 코드의 의미
```
	----------------------------------------------------
	<1>arr 변수 선언하기
	----------------------------------------------------
	<2>Array 객체 생성한 후 Array 객체 메위주를 arr 변수안에 저장하기
		변수에 메위주를 넣는 이유는 재사용하기 위함이다.
	----------------------------------------------------

	이때 Array 객체 안에는 배열변수가 0개이다. 즉 배열변수는 없다. 추후 저장
	다량의 데이터를 자장할 목적으로 Array 객체를 생성해 놓은 것이다.
	----------------------------------------------------
	위 코드는 아래와 같은 코드이다.
	var arr = new Array();
```
### <28> 자스에서 var arr = [1,3,5]; 이 코드의 의미
```
	----------------------------------------------------
	<1>arr 변수 선언하기
	----------------------------------------------------
	...
	이때 Array 객체 안에는 배열변수가 3개이다.
	3개의 배열변수 안에는 1,3,5 가 저장되어 있다.
	----------------------------------------------------
	각 배열변수를 호출하는 방법은 arr[0] arr[1] arr[2] 이다.
	대괄호안에 인덱스 번호를 쓸수 있으므로
	반복문을 돌리면서 이 인덱스 번호를 바꾸어 전급한 수 있다.

```
### <29> 자스에서 var xxx = getDdd(3); 이 코드의 의미
```
	----------------------------------------------------
	<1>xxx 변수 선언하기
	----------------------------------------------------
	...
	----------------------------------------------------
	<주의>함수를 호출한 지정이 = 오른쪽이다. = 오른쪽은 데이터가 나와야 하는 곳이므로
			이 함수는 호출하면 반드시 데이터를 리턴할 것이다.
	<참고> 관용적으로 get 으로 시작하는 함수는 리턴값이 있다.
	<참고> 관용적으로 set 으로 시작하는 함수는 리턴값이 없다.
```

### <30> 자스에서 var xxx = new Date(); 이 코드의 의미
```
	----------------------------------------------------
	<1> xxx 변수 선언하기
	----------------------------------------------------
	<2> Date 객체 생성한 후 Date 객체의 메위주를 xxx 변수에 저장하기.
		...
		현재란 지금 이 시점에 년월일시분초를 말한다. 즉 흘러가는 년월일시분초를 말한다.
	----------------------------------------------------
	객체 생성후 아마 xxx.메소드() 형식으로 Date 객체의 메소드를 호출할 것이다.
	객체 생성의 목적은 메소드 호출이다.
```

### <31> 자스에서 var xxx = new Date(1999,11,25); 이 코드의 의미
```
	----------------------------------------------------
	<1> xxx 변수 선언하기
	----------------------------------------------------
	<2> Date 객체 생성한 후 Date 객체의 메위주를 xxx 변수에 저장하기.
		이때 Date 객체는 1999년 12월 25일 0시0분0초를 관리하고 있다. 
		고정된 1999년 12월 25일 0시0분0초를 관리하고 있다.
	----------------------------------------------------
	<주의> Date 객체 시 월은 사람이 생각하는 월에서 -1 한 월을 세팅한다
			Date 객체는 월을 0~11 까지의 숫자로 관리하고 있기 때문이다.
	----------------------------------------------------
	<요령> 월 세텡 헷갈리므로 아래 처럼하면 덜 헷갈린다.
			--------------------------------------
			var xx = new Date(1999,12-1,25);
```

### <32> 자스에서 var xxx = new Date(1999,13,25); 이 코드에서 Date 객체 관리하는 년월일이 뭘까?
```
	위 코드를 아래 처럼 변경하면
	-------------------------------------
	...
	-------------------------------------
	11은 12월을 의미한다.
	12월에 2개월 대한 것과 마찬가지이므로 결국 2000년 2월 25일 0시 0분 0초을 관리한다.
	<참고> 시분초가 없으면 0시 0분 0초가 세팅된 것과 동일하다.

```

### <33> 자스에서 var xxx = function(a){~}; 이 코드는?
```
	----------------------------------------------------
	<1> xxx 변수 선언하기
	----------------------------------------------------
	...
	----------------------------------------------------
	자스 익명함수 즉 이름없는 함수도 데이터 취급을 하므로 변수에 저장이 가능하다.
	----------------------------------------------------
	익명함수도 함수 호출이 목적이다.
	----------------------------------------------------
	익명함수를 변수에 저장하므로 이동하기 쉽고 필요시 호출할 수 있다.
	----------------------------------------------------
...
### <34> html 파일 안에서 <script src="common_func.js"></script> 이 코드는?
```
	-------------------------------------------------------------------------
	common_func.js 파일안의 자바스크립트 소스를 복사해서 현 script 태그 사이에 삽입하라.
	-------------------------------------------------------------------------
	그로므로 <script src="common_func.js"></script> 를 가지고 있는 html 파일안에
	common_func.js 파일안에 자바스크립트 소스가 있는 것과 동일하다.
	-------------------------------------------------------------------------
	script src="common_func.js" 의미는 현재 이 html 파일의 저장 폴더와 동일한 폴더에 common_func.js 파일도 존재한다는 의미다.
	즉 JS 파일 이름만 나온다면 동일한 폴더에 들어 있다는 의미이다.
```

### <35> 호출할 함수는 호출 코드 위에만 나와야하는가?
```
	-------------------------------------------------------------------------
	아니다.
	-------------------------------------------------------------------------
	함수호출 코당이 먼저 나오고 함수 선언이 나중에 나와도 된다.
	-------------------------------------------------------------------------
	웹브라우저는 html 파일 안의 html 소스 또는 자스 소스를 읽어들일 때 아래와 규칙을 사용한다.
		-----------------------------------------------------------------------------------
		<1> 먼저 정적함수 즉, function 함수명(~){~} 이렇게 생긴 함수를 먼저 골라 읽어 들인다.
		-----------------------------------------------------------------------------------
		<2> 정적함수를 제외하고 맨위에서 부터 html 소스 또는 자스 소스를 차례대로 읽어 들여 실행한다.
		-----------------------------------------------------------------------------------
	그렇지만 정적함수는 즉 이름있는 함수는 관용적으로 맨위 head 태그 안에 선언하는 게 좋다.

	<참고> 정적함수는 선언은 function 함수명(~){~} 를 말한다.
```
...
```
	...
	-----------------------------------------
	--------------------------------------------------------------------------------------------------------------
	조건문인 if 문에서 if 오른쪽은 true 또는 false 를 리턴하는 [비교연산식] 또는 [논리연산식] 또는 [함수 호출] 이 나온다.
	--------------------------------------------------------------------------------------------------------------
	if( true 또는 false 를 리턴하는 [비교연산식] 또는 [논리연산식] 또는 [함수 호출]){
		실행구문;
	}
	--------------------------------------------------------------------------------------------------------------
...
```
	...
	------------------------------------------------------------------------------------
	document.write(~) => Document 객체의 write메소드를 호출한 것이다.
							변수 document 안에는 Document 객체가 이미 저장되어 있다.
							괄호 안의 문자열을 html 태그 소스 또는 html 문자열로 html 파일에 출력
							즉 <script>document.write("문자열")</script> 이 코드가 나오는 자리에
							문자열이 들어간다.
							자스 변수안에 데이터 또는 함수리턴값을 html에 출력하고 싶을때 씀
```
